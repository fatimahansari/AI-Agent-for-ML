...
def main():
    ...
    try:
        ...
        model_1._linear_regression, scaler_1._linear_regression = train_and_save_model(
        ...
        print("="*70)
        print(f"Trained {'1. Linear Regression'} on full dataset")
        print(f"Model saved in: {output_dir}/")
        safe_model_name = "1. Linear Regression".lower().replace(' ', '_').replace('-', '_')
        print("\nSaved files:")
        print(f"  - model_{safe_model_name}.pkl")
        if True:
            print(f"  - scaler_{safe_model_name}.pkl")
        model_name_lower = "1. Linear Regression".lower()
        if 'xgboost' in model_name_lower or 'lightgbm' in model_name_lower:
